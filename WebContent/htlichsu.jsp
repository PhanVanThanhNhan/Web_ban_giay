		...
		<div class="header_ctn">
			<ul class="navlist">
				...
				<c:if test="${DangNhap==null }">
					<div class="log">
						<li class="login"><a class="list_nav" href="ktdn">Đăng
								nhập</a></li>
						<li class="signin"><a class="list_nav"
							href="khachhangController">Đăng ký</a></li>
					</div>
				</c:if>
				<c:if test="${DangNhap != null }">
							...
							<tbody>
								<c:forEach var="h" items="${ls }">
									<tr>
										...
										<td data-th="STT"><c:out value="${i }" /></td>

										<td data-th="Sản phẩm"><img src="${h.getAnh() }" alt=""
											class="img_book"></td>
										<td data-th="Tên giày">
											<h4>${h.getTengiay() }</h4>
										</td>
										...
										<td data-th="Số tiền" style="color: orangered">
											<h4>${h.getGia() * h.getSoluong() }VNĐ</h4>
										</td>
										<td data-th="Ngày mua">
											<h4>
												<fmt:formatDate value="${h.getNgaymua() }"
													pattern="dd/MM/yyyy" />
											</h4>
										</td>
										<td data-th="Trạng thái"><c:if test="${h.isDamua() == false}">
												<h4>Chưa thanh toán</h4>
											</c:if> <c:if test="${h.isDamua() != false}">
												<h4>Đã thanh toán</h4>
											</c:if></td>
