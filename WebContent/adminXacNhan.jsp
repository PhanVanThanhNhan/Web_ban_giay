		...
		<table class="tb_gh"
			...
			<thead>
					<tr>
						...
						<th style="width: 16%">Ngày mua</th>
						<th style="width: 10%; align-items: center">Thao tác</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${xn }" var="d">
						<tr>
							<td data-th="Tên khách hàng">${d.getHoten()}</td>
							<td data-th="Tên giày">${d.getTengiay()}</td>
							<td data-th="Giá">${d.getGia() }</td>
							<td data-th="Số lượng">${d.getSoluongmua()}</td>
							<td data-th="Số tiền">${d.getThanhtien() }</td>
							
							
							<td data-th="Ngày mua">${d.getNgaymua()}</td>
							<td data-th="Thao tác"><a class="thaotac"
								href="adminXacNhanController?hd=xacnhan&mcthd=${d.getMacthd() }">
									xác nhận </a> 
						</tr>
					</c:forEach>
